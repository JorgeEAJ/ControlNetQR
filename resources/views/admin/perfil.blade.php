...
<body class="bg-gray-100 flex items-center justify-center min-h-screen dark:bg-zinc-900">
    ...
    <form action="{{ route('admin.perfil.update') }}" method="POST" class="bg-white dark:bg-black p-8 rounded-xl shadow-md w-96">
        @csrf
        @method('PUT')

        <h2 class="text-2xl font-bold mb-6 text-center text-blue-600">Perfil del Administrador</h2>
        ...
        <a href="{{ route('panel.admin') }}" class="block text-center mt-4 bg-gray-300 hover:bg-gray-400 text-black font-semibold py-2 px-4 rounded transition">
            Regresar
        </a>
    </form>
</body>
